...
def construir(fwd_val: pd.DataFrame | None = None,
              ...
              swaps_sabana: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    if swaps_sabana is not None and len(swaps_sabana):
        # Una fila por pata (derecho/obligacion); valor de mercado pendiente (VPN v6).
        for _, r in swaps_sabana.iterrows():
            ...
            filas.append(_fila(etq, r, tipo, r.get("der_moneda", ""),
                               r.get("der_nominal", 0), None, r.get("der_tasa_facial", None), False))
            filas.append(_fila(etq, r, tipo, r.get("obl_moneda", ""),
                               r.get("obl_nominal", 0), None, r.get("obl_tasa_facial", None), False))
    if not filas:
